...
status_t ResStringPool::setTo(const void* data, size_t size, bool copyData)
{
    ...
    if (mHeader->stringCount > 0) {
        ...
        if (mHeader->flags&ResStringPool_header::UTF8_FLAG) {
            ...
            mCache = (uint16_t**)malloc(sizeof(uint16_t**)*mHeader->stringCount);
            memset(mCache, 0, sizeof(uint16_t**)*mHeader->stringCount);
        } else {
...
ssize_t ResStringPool::indexOfString(const uint16_t* str, size_t strLen) const
{
    if (mError != NO_ERROR) {
        return mError;
    }
    ...
    if (mHeader->flags&ResStringPool_header::SORTED_FLAG) {
        ...
        while (l <= h) {
            mid = l + (h - l)/2;
            const uint16_t* s = stringAt(mid, &len);
        ...
        for (int i=mHeader->stringCount-1; i>=0; i--) {
            const uint16_t* s = stringAt(i, &len);
...
ssize_t ResXMLParser::indexOfAttribute(const uint16_t* ns, size_t nsLen,
                                       const uint16_t* attr, size_t attrLen) const
{
    if (mEventCode == START_TAG) {
        const size_t N = getAttributeCount();
        for (size_t i=0; i<N; i++) {
            size_t curNsLen, curAttrLen;
            const uint16_t* curNs = getAttributeNamespace(i, &curNsLen);
            const uint16_t* curAttr = getAttributeName(i, &curAttrLen);
...
const uint16_t* ResTable::valueToString(
    const Res_value* value, size_t stringBlock,
    uint16_t tmpBuffer[TMP_BUFFER_SIZE], size_t* outLen)
{
    if (!value) {
        return NULL;
    }
...
uint32_t ResTable::identifierForName(const uint16_t* name, size_t nameLen,
                                     const uint16_t* type, size_t typeLen,
...
nope:
    ...
    const uint16_t* packageEnd = NULL;
    const uint16_t* typeEnd = NULL;
    const uint16_t* const nameEnd = name+nameLen;
    const uint16_t* p = name;
    while (p < nameEnd) {
        if (*p == ':') packageEnd = p;
        else if (*p == '/') typeEnd = p;
        p++;
                                 ...
                                 bool* outPublicOnly)
{
    const uint16_t* packageEnd = NULL;
    const uint16_t* typeEnd = NULL;
    const uint16_t* p = refStr;
    const uint16_t* const end = p + refLen;
    while (p < end) {
        if (*p == ':') packageEnd = p;
        else if (*p == '/') {
            typeEnd = p;
...
bool ResTable::stringToInt(const uint16_t* s, size_t len, Res_value* outValue)
{
...
bool ResTable::stringToFloat(const uint16_t* s, size_t len, Res_value* outValue)
{
    while (len > 0 && isspace16(*s)) {
        s++;
        len--;
                             ...
                             bool enforcePrivate) const
{
    ...
    if (*s == '@') {
        ...
        if (len == 5 && s[1]=='n' && s[2]=='u' && s[3]=='l' && s[4]=='l') {
            ...
            const uint16_t* resourceRefName;
            int resourceNameLen;
            if (len > 2 && s[1] == '+') {
    ...
    if ((attrType&ResTable_map::TYPE_FLAGS) != 0) {
        ...
        if (cnt >= 0) {
            ...
            outValue->data = 0;
            const uint16_t* end = s + len;
            const uint16_t* pos = s;
            while (pos < end && !failed) {
                const uint16_t* start = pos;
                pos++;
                while (pos < end && *pos != '|') {
                    pos++;
                }
...
const uint16_t* ResTable::getBasePackageName(size_t idx) const
{
    if (mError != NO_ERROR) {
        return 0;
    }
